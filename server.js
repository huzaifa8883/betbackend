...
const { autoMatchPendingBets } = require('./routes/Orders');
const app = express();
...
io.on("connection", (socket) => {
  ...
  socket.on("updateMarket", async (data) => {
    ...
    if (data.marketId) {
      try {
        // If selectionId is provided, auto-match for that specific selection
        if (data.selectionId) {
          await autoMatchPendingBets(data.marketId, data.selectionId);
        } else {
          // Otherwise, auto-match for all selections in the market
          // Use the same pattern as Orders.js
...
setInterval(async () => {
  ...
  try {
    ...
    // 🔹 After market update, trigger auto-matching for pending bets
    // This is handled within updateMarkets function now
  } catch (err) {
